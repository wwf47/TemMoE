...
from src.models.model_utils.clamp_utils import Clamp
...
class XGLMModel(XGLMPreTrainedModel):
    ...
    def forward(
        ...
        # past_key_values_length
        # NOTE: during HF `generate()` with `use_cache=False`, some transformer versions still pass
        # `past_key_values` as a structure containing Nones. Treat that as "no cache".
        past_key_values_length = 0
        if past_key_values is not None:
            try:
                pkv0 = past_key_values[0]
                if pkv0 is not None and pkv0[0] is not None:
...
class XGLMForCausalLM(XGLMPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # When KV cache is available, only feed the last token
        if past_key_values is not None:
            try:
                pkv0 = past_key_values[0]
                if pkv0 is not None and pkv0[0] is not None:
    ...
    def _reorder_cache(past_key_values, beam_idx):
        # HF generation may call this even when `use_cache=False`, with cache entries being None.
        # Preserve structure and safely pass through Nones.
        ...
        for layer_past in past_key_values:
            ...
            for past_state in layer_past:
                if past_state is None:
                    ...
                    reordered_layer.append(past_state.index_select(0, beam_idx))
            reordered_past += (tuple(reordered_layer),)
